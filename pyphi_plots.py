...
def timestamp():
    """
    Returns a timestamp, for use in figure filenames.
    """
...
def r2pv(mvm_obj,*,plotwidth=600,plotheight=400,addtitle='',material=False,zspace=False):
    ...
    if is_pls:
        output_file("r2xypv_"+timestamp()+".html",title="R2"+ yaxlbl+ "YPV",mode='inline') 
        ...
        output_file("r2xpv_"+timestamp()+".html",title='R2XPV',mode='inline') 
        #colormap =cm.get_cmap("rainbow")
        colormap = matplotlib.colormaps['rainbow']
        different_colors=A
        color_mapping=colormap(np.linspace(0,1,different_colors),1,True)
...
def loadings(mvm_obj,*,plotwidth=600,xgrid=False,addtitle='',material=False,zspace=False):
    ...
    if is_pls:
        output_file("Loadings_"+space_lbl+"_Space_"+timestamp()+".html",title=space_lbl+' Loadings PLS',mode='inline')
        for i in list(np.arange(A)):
            ...
            if i==0:
                ...
                p_list.append(p)
        show(column(p_list))    
        output_file("Loadings_Y_Space_"+timestamp()+".html",title='Y Loadings PLS',mode='inline')
        for i in list(np.arange(A)):
        ...
        output_file("Loadings_X_Space_"+timestamp()+".html",title='X Loadings PCA',mode='inline') 
        for i in list(np.arange(A)):
...
def loadings_map(mvm_obj,dims,*,plotwidth=600,addtitle='',material=False,zspace=False,textalpha=0.75):
    ...
    if 'Q' in mvmobj:
        ...
        output_file("Loadings_Map_"+timestamp()+".html",title='Loadings Map',mode='inline')
...
def weighted_loadings(mvm_obj,*,plotwidth=600,xgrid=False,addtitle='',material=False,zspace=False):
    ...
    if is_pls:
        output_file("Loadings_"+space_lbl+"_Space_"+timestamp()+".html",title=space_lbl+' Weighted Loadings PLS',mode='inline')
        for i in list(np.arange(A)):
            ...
            if i==0:
                ...
                p_list.append(p)
        show(column(p_list)) 
        output_file("Loadings_Y_Space_"+timestamp()+".html",title='Y Weighted Loadings PLS',mode='inline')
        for i in list(np.arange(A)):
        ...
        output_file("Loadings_X_Space_"+timestamp()+".html",title='X Weighted Loadings PCA',mode='inline') 
        for i in list(np.arange(A)):
...
def vip(mvm_obj,*,plotwidth=600,material=False,zspace=False,addtitle=''):
    ...
    if 'Q' in mvmobj:  
        if (mvmobj['type']=='lpls') or (mvmobj['type']=='jrpls') or (mvmobj['type']=='tpls'):
            ...
            num_varX=mvmobj['P'].shape[0] 
        output_file("VIP_"+timestamp()+".html",title='VIP Coefficient',mode='inline') 
...
def score_scatter(mvm_obj,xydim,*,CLASSID=False,colorby=False,Xnew=False,
    ...
    if isinstance(CLASSID,bool): # No CLASSIDS
        output_file("Score_Scatter_"+timestamp()+".html",title='Score Scatter t['+str(xydim[0])+'] - t['+str(xydim[1])+ ']',mode='inline')
...
def score_line(mvmobj,dim,*,CLASSID=False,colorby=False,Xnew=False,add_ci=False,add_labels=False,add_legend=True,plotline=True,plotwidth=600,plotheight=600):
    ...
    if isinstance(CLASSID,bool): # No CLASSIDS
        output_file("Score_Line_"+timestamp()+".html",title='Score Line t['+str(dim[0])+ ']',mode='inline')
...
def diagnostics(mvmobj,*,Xnew=False,Ynew=False,score_plot_xydim=False,plotwidth=600,ht2_logscale=False,spe_logscale=False):
    ...
    output_file("Diagnostics_"+timestamp()+".html",title='Diagnostics',mode='inline') 
...
def predvsobs(mvmobj,X,Y,*,CLASSID=False,colorby=False,x_space=False):
    ...
    if isinstance(CLASSID,bool): # No CLASSIDS
        output_file("ObsvsPred_"+timestamp()+".html",title='ObsvsPred',mode='inline')
        plot_counter=0
...
def contributions_plot(mvmobj,X,cont_type,*,Y=False,from_obs=False,to_obs=False,lv_space=False,plotwidth=800,plotheight=600,xgrid=False):
    ...
    output_file("Contributions_"+timestamp()+".html",title='Contributions',mode='inline')
...
def plot_spectra(X,*,xaxis=False,plot_title='Main Title',tab_title='Tab Title',xaxis_label='X- axis',yaxis_label='Y- axis'): 
    ...
    if isinstance(X,pd.DataFrame):
        ...
        if isinstance(xaxis,np.ndarray):
            ...
            x=np.reshape(x,(1,-1))
    output_file("Spectra_"+timestamp()+".html",title=tab_title,mode='inline')
...
def plot_line_pd(X,col_name,*,plot_title='Main Title',tab_title='Tab Title',xaxis_label='X- axis',plotheight=400,plotwidth=600): 
    ...
    output_file("LinePlot_"+timestamp()+".html",title=tab_title,mode='inline')
...
def mb_weights(mvmobj,*,plotwidth=600,plotheight=400):
    ...
    for i in list(np.arange(A)):
        output_file("blockweights_"+timestamp()+".html",title="Block Weights",mode='inline')         
...
def mb_r2pb(mvmobj,*,plotwidth=600,plotheight=400):
    ...
    for i in list(np.arange(A)):
        ...
        output_file("r2perblock_"+timestamp()+".html",title="R2 per Block",mode='inline') 
        #colormap =cm.get_cmap("rainbow")
        colormap = matplotlib.colormaps['rainbow']
        different_colors=A
        color_mapping=colormap(np.linspace(0,1,different_colors),1,True)
...
def mb_vip(mvmobj,*,plotwidth=600,plotheight=400):
    ...
    XVar = XVar_
    vip=vip[index]
    output_file("blockvip_"+timestamp()+".html",title="Block VIP",mode='inline') 
